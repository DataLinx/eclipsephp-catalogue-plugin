<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    ...
    >
        ...
        <div x-show="state && state.length > 0" class="space-y-4">
            <div
                x-sortable
                wire:end.stop="reorderImages('{{ $getStatePath() }}', $event.target.sortable.toArray())"
                class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4"
            >
                <template x-for="(image, index) in state" :key="`${image.uuid}-${currentLocale}`">
                    <div
                        x-sortable-item
                        :x-sortable-item="image.uuid || index"
                        class="relative group bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden transition-all hover:shadow-md"
                    >
                        <div
                            x-sortable-handle
