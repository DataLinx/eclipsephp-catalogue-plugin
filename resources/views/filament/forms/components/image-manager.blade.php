        ...
        <x-filament::section compact>
            ...
            <div x-data="imageManager({
                ...
                <div x-show="state && state.length > 0" class="space-y-4">
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        <template x-for="(image, index) in state" :key="image.uuid">
                            <div class="relative group bg-white dark:bg-gray-800 rounded-lg shadow-sm border overflow-hidden transition-all duration-300"
                                :class="{
                                    'opacity-50': draggingIndex === index,
                                    'border-gray-200 dark:border-gray-700 hover:shadow-md': dropTargetIndex !== index,
                                    'border-primary-500 dark:border-primary-400 shadow-lg': dropTargetIndex === index &&
                                        draggingIndex !== index,
                                    'cursor-move': true
                                }"
                                draggable="true" @dragstart="dragStart($event, index)"
                                @dragenter.prevent="dragEnter($event, index)"
                                @dragover.prevent="dragOver($event, index)" @dragleave="dragLeave($event, index)"
                                @drop="drop($event, index)" @dragend="dragEnd($event)">
                                <div class="absolute top-2 left-2 z-20 opacity-0 group-hover:opacity-100 transition-opacity cursor-move"
                                ...
                                <div x-show="image.is_cover"
                                    class="absolute top-2 right-2 z-20 bg-primary-500 text-white text-xs px-2 py-1 rounded-full font-medium shadow-sm">
                                    Cover
                                </div>

                                <div x-show="dropTargetIndex === index && draggingIndex !== index"
                                    x-transition:enter="transition ease-out duration-200"
                                    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                                    class="absolute inset-0 bg-primary-500/10 dark:bg-primary-400/10 z-10 pointer-events-none">
                                    <div
                                        class="absolute inset-0 border-2 border-dashed border-primary-500 dark:border-primary-400 rounded-lg m-2">
                                    </div>
                                </div>

                                <div class="aspect-square bg-gray-100 dark:bg-gray-900 cursor-pointer"
                                ...
                                <div class="p-3 space-y-2">
                                    ...
                                    <p class="text-xs text-gray-600 dark:text-gray-400 line-clamp-2"
                                        x-text="getLocalizedDescription(image)" x-show="getLocalizedDescription(image)">
                                    </p>
                                    <div class="flex flex-wrap items-center gap-1">
                                        ...
                                        <button type="button" @click="handleSetCover(image)"
                                            class="text-xs font-medium inline-flex items-center gap-1"
                                            :class="image.is_cover ? 'text-gray-500 dark:text-gray-400 cursor-not-allowed' :
                ...
                <div x-show="!state || state.length === 0" class="text-center py-12">
                    ...
                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Click "Upload Files" or "Add from URL" to
                        get started</p>
                </div>
            ...
            function imageManager({
                ...
                return {
                    ...
                    handleSetCover(image) {
                        ...
                        this.$wire.mountFormComponentAction('{{ $getStatePath() }}', 'setCover', {
                            arguments: {
                                uuid: image.uuid
                            }
                        });
                    ...
                    drop(event, dropIndex) {
                        ...
                        this.$wire.mountFormComponentAction('{{ $getStatePath() }}', 'reorder', {
                            items: newOrder
                        });
